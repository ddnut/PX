...
// PX: optional setting to unlock wallet for block minting only;
...
// PX: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // PX: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // PX: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // PX: not avatar mode
                        {
...
// PX: create coin stake transaction
...
// PX: check 'spent' consistency between wallet and txindex
// PX: fix wallet spent state according to txindex
...
// PX: disable transaction (only for coinstake)
